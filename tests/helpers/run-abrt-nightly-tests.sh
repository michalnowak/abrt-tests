...
echo "Install VM: $VM"
virt-install --name "$VM" --ram "1222" --location "http://download.fedoraproject.org/pub/fedora/linux/releases/15/Fedora/x86_64/os/" --disk path=/home/newman/$VM.img,size=4,sparse=true --accelerate -x "ks=http://mnowak.fedorapeople.org/anaconda-ks.cfg" --graphics type=vnc --noautoconsole
